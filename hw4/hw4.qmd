...
rf_model <- train(as.factor(thirty_day_mort)~ .,
                           data = icu_train,
                           method = "rf",
                           tuneLength = 25,
                           trControl = control)
...
pred_rf <- predict(rf_model, icu_test)
...
svm_model <- train(as.factor(thirty_day_mort)~ .,
                           data = icu_train,
                           method = "svmRadial", 
                           tuneLength = 25,                  
...
pred_svm <- predict(svm_model, icu_test)
