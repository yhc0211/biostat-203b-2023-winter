...
`echo 'hello, world' > test1.txt` directs 'hello, world' from command to test1.txt.
...
```{bash, echo=T}
w
#it is used to show who is logged on and 
```
...
```{bash, echo=T}
time sleep 5
#Used to delay for a fixed amount of time during the execution of any script. Here I delay 5 seconds
```


```{bash, echo=T}
history | tail
#show a list of commands I used when I started the session
```
